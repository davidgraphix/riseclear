...
const SERVICES = [
  "Residential Cleaning — Standard",
  "Residential Cleaning — Deep Clean",
  "Residential Cleaning — Move-In / Move-Out",
  "Commercial Cleaning",
  "Window Cleaning",
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        <div className="text-center mb-14" ref={ref}>
          ...
          <motion.p initial={{ opacity:0,y:14 }} animate={inView ? { opacity:1,y:0 } : {}} transition={{ delay:0.14 }}
            ...
            Fill in the form, call us, or DM us on Instagram. We respond to all inquiries within a few hours.
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-14 max-w-6xl mx-auto">
          ...
          <motion.div initial={{ opacity:0, x:-24 }} animate={inView ? { opacity:1,x:0 } : {}} transition={{ delay:0.2, duration:0.6 }} className="lg:col-span-2 space-y-4">
            {[
              { href:"tel:+14318164106",           icon:Phone, label:"Call or Text",  value:"+1 431 816 4106",    sub:"Mon–Sat, 8am–6pm CST" },
              { href:"mailto:info@risecleaning.ca", icon:Mail,  label:"Email Us",      value:"info@risecleaning.ca", sub:"Reply within a few hours" },
            ].map(({ href, icon:Icon, label, value, sub }) => (
              <a key={label} href={href} className="group flex items-center gap-4 card p-5 hover:border-sky-300" aria-label={label}>
          ...
          <motion.div initial={{ opacity:0, x:24 }} animate={inView ? { opacity:1,x:0 } : {}} transition={{ delay:0.3, duration:0.6 }} className="lg:col-span-3">
            <div className="bg-white rounded-3xl border border-sky-200 shadow-[0_8px_40px_rgba(14,165,233,0.10)] overflow-hidden">
              <div className="px-6 sm:px-8 py-5 border-b border-sky-100" style={{ background:"linear-gradient(135deg,#F0F9FF,#E0F2FE)" }}>
                <h3 className="font-display font-bold text-lg text-brand-ink">Tell us about your needs</h3>
                ...
                {success ? (
                  ...
                  <form onSubmit={handleSubmit} noValidate>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                      <div>
                        <label htmlFor="c-name" className={lbl}>Name <span className="text-sky-500 normal-case font-normal">*</span></label>
                        <input id="c-name" type="text" value={form.name} onChange={set("name")} placeholder="Jane Smith" required autoComplete="name" className="input-field" />
                      </div>
                      <div>
                        <label htmlFor="c-email" className={lbl}>Email <span className="text-sky-500 normal-case font-normal">*</span></label>
                        <input id="c-email" type="email" value={form.email} onChange={set("email")} placeholder="jane@example.com" required autoComplete="email" className="input-field" />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                      <div>
                        <label htmlFor="c-phone" className={lbl}>Phone</label>
                        <input id="c-phone" type="tel" value={form.phone} onChange={set("phone")} placeholder="+1 (204) 555-0123" autoComplete="tel" className="input-field" />
                      </div>
                      <div>
                        <label htmlFor="c-service" className={lbl}>Service Needed</label>
                        <select id="c-service" value={form.service} onChange={set("service")} className="input-field cursor-pointer appearance-none">
                    ...
                    <div className="mb-6">
                      <label htmlFor="c-msg" className={lbl}>Message <span className="text-sky-500 normal-case font-normal">*</span></label>
                      <textarea id="c-msg" value={form.message} onChange={set("message")} rows={5} required className="input-field resize-none"
                        placeholder="Tell us about your property, preferred schedule, frequency, or any questions…" />
                    </div>
                    {error && (
